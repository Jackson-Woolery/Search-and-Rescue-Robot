  ...
  switch (state)
  {
   ...
   case 3:
          ...
          if(distanceRead != 0){ // Once distance changes, indicates Pi sent accurate distance
            distanceToGo = (distanceRead / (float) 12) - (float) 1.3; // Converts Pi distance to feet, then accounts for stopping away from the beacon
   ...
   case 6:
            ...
            if(abs(angularPositionR) >= (float) 44.7){      // 46.5 is number of radians the outer wheel needs to turn (adjusts how much of the circle is completed)
              state = 7;
            }
        break;
